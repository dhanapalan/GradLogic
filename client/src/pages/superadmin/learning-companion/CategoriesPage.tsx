...
export default function CategoriesPage() {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const category = await questionBankService.createCategory(newCategory.name, newCategory.description);
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight text-gray-900 flex items-center gap-2">
            <FolderTree className="w-6 h-6 text-admin-accent" />
            Categories
          </h2>
          <p className="text-gray-500 mt-1">Curated taxonomy powering the Knowledge Library.</p>
        </div>
        <button
        ...
        <div className="bg-white rounded-xl border border-gray-200/70 shadow-admin-card p-6 mb-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={handleAddCategory} className="px-4 py-2 bg-navy-900 text-white rounded-lg font-medium hover:bg-navy-800">
                Add
              </button>
              <button onClick={() => setShowAddForm(false)} className="px-4 py-2 border border-gray-300 rounded-lg font-medium hover:bg-gray-50">
                Cancel
              </button>
            </div>
          </div>
          ...
          {categories.map((category) => (
            <div key={category.id} className={`bg-white rounded-xl shadow-admin-card border p-6 ${category.is_active === false ? "border-gray-200/70 opacity-75" : "border-gray-200/70"}`}>
              <div className="flex items-start justify-between mb-4">
                ...
                {editingId !== category.id && (
                  <div className="flex gap-2">
                    <button onClick={() => startEdit(category)} className="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors">
                      <Pencil className="w-4 h-4" />
                    </button>
                    {category.is_active === false ? (
                      <button onClick={() => handleActivateCategory(category.id)} className="px-3 py-1.5 text-sm text-green-700 border border-green-300 rounded-lg hover:bg-green-50">
                        Activate
                      </button>
                    ) : (
                      <button onClick={() => handleDeactivateCategory(category.id)} className="px-3 py-1.5 text-sm text-red-700 border border-red-300 rounded-lg hover:bg-red-50">
                        Deactivate
                      </button>
                    )}
                  </div>
